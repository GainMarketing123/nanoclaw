...
- **2026-07-31 enforcement-propagation fix (`CLAUDE_SETTINGS_SOURCE_DIR`):** the container rulebook source is now a SEPARATE constant from the credential root. `CLAUDE_CONFIG_DIR`/`HOST_CLAUDE_DIR` = where the CREDENTIALS live (credential-proxy only); `CLAUDE_SETTINGS_SOURCE_DIR` = where the RULES live (`writeContainerSettings` only). Defaults to `HOME_DIR/.claude`, which the `nanoclaw.service` EnvironmentFile pins to `/home/atlas` — correct on the VPS with no unit change; override with the `CLAUDE_SETTINGS_SOURCE_DIR` env var. **Why:** the 2026-07-11 shared-identity OAuth cutover added `Environment=CLAUDE_CONFIG_DIR=/home/nanoclaw-he/.claude` (`infra/systemd-dropins/nanoclaw.service.d/oauth-shared-identity.conf`, installed on the VPS 2026-07-11 19:14 EDT). `container-runner.ts` located the rulebook through that same constant, but the rulebook never moved from `/home/atlas/.claude/settings.json`. `/home/nanoclaw-he/.claude/settings.json` is `{}`, so the container-spawn parity gate compared 62 manifest-required hooks against zero registered ones, refused to propagate (correctly), and froze every channel on its last-good set — logging `container_settings_parity_refused` on every spawn for three weeks. Measured live 2026-07-31 pre-fix: `atlas_teams` 54, `atlas_gpg` 18 (7 pointing at deleted scripts), `atlas_main` 17, `telegram_atlas-marketing` 0, `atlas_crownscape`/`atlas_wisestream` never provisioned; host rulebook 62. **The parity gate was NOT weakened** — it is the thing that caught this. Regression tests: `src/container-settings-source.test.ts` (red-first, plus a control proving the gate still fails CLOSED on a hook-less source). **Propagation is LAZY** — `writeContainerSettings()` runs only at a group's next container spawn (`buildVolumeMounts` ← `runContainerAgent`), so a restart does not re-propagate; each channel picks up the full set at its next use. Deploy/rollback runbook (measured permissions, revert-then-restore ordering): `plans/enforcement-propagation-deploy-rollback-runbook.md`.
